...
export function Modal({ project, onClose }: ModalProps) {
  ...
  return (
    ...
    >
      ...
      >
        {/* header */}
        <div className="sticky top-0 z-10 backdrop-blur-md bg-raised/85 border-b border-line/[0.08] px-6 py-4 flex items-start justify-between gap-4">
        ...
        <div className="px-6 pt-6">
          {project.coverImage && (
            <div className="rounded-2xl overflow-hidden border border-line/[0.08] mb-5">
              <img src={project.coverImage} alt={project.title} className="w-full h-48 sm:h-60 object-cover" />
            </div>
          )}
        ...
        {/* body */}
        <div className="p-6 space-y-9 mt-2">
          {project.businessProblem?.length > 0 && (
            <Block label="Problem" title="The Problem"><List items={project.businessProblem} plain /></Block>
          )}
          {project.businessObjective?.length > 0 && (
            <Block label="Goal" title="The Objective"><List items={project.businessObjective} plain /></Block>
          )}
          ...
          {project.projectArchitecture?.description && (
            <Block label="Arch" title="Architecture">
              <div className="rounded-2xl border border-line/[0.08] bg-line/[0.02] p-5">
                <List items={project.projectArchitecture.description} />
              </div>
          ...
          {project.methodology?.length > 0 && (
            <Block label="Build" title="How I Built It"><List items={project.methodology} plain /></Block>
          )}
